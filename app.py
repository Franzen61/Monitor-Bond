...
try:
    ...
    with c1:
        ...
        # Etichetta Regime (Immagine 2, riga 17)
        regime_move = "⚠️ REGIME DIPENDENTE DAL MOVE" if s_move < 1 else "✅ REGIME ROBUSTO (non dipende dal MOVE)"
        st.write(f"**{regime_move}**")
    ...
    # --- SEZIONE RATIOS TESTUALI ---
    st.divider()
    r1, r2, r3 = st.columns(3)
    r1.metric("Duration Confidence", f"{dur_conf:.1%}")
    
    # Testo Stabilità (Immagine 2, riga 22)
    ...
    r2.metric("Signal Stability", f"{sig_stab:.1%}", help=stab_txt)
    ...
    with g1:
        fig_ry = go.Figure()
        fig_ry.add_trace(go.Scatter(x=d['ry_hist'].index, y=d['ry_hist'].values, name="10Y Real Yield", line=dict(color='#00ff00')))
